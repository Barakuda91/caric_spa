(function() {
  ...
  function Service() {
    ...
    this.defSelectCheck = function(param) {
      if(param === defaultParameterKeyName) {
        return '';
      } else {
        return param;
      }
...
>>>>>>> ba6cf7dac816538742eabcae4dd6d012d69f8f2c
    ...
    this.getSettingParameter = function(parameter,disabled) {
      ...
      if(disabled) {
        parameter.unshift({title: 'PLEASE_SELECT', disabled: false, key: defaultParameterKeyName});
      }
      return parameter;
    };
      ...
      for(var setting in settingParams) {
        if(settingParams[setting].length > 0) {
          ...
          if (settingParams[setting][1]) {
            defEl = (settingParams[setting][1].title) ? settingParams[setting][1].title : settingParams[setting][1];
          }

          if (typeof settingParams[setting] == 'string') {
            return_[setting] = settingParams[setting];
          } else {
            return_[setting] = (useSettingParams && defaultValues[setting]) ? defaultValues[setting] : settingParams[setting][0].title;
          }
        }
      }
      return return_;
      ...
      this.createAdvertTitleByType = function (obj) {
        ...
        if (obj.advertType) {
          switch (obj.advertType) {
            case 'wheels':
              title = [obj.pcd, 'R'+obj.diameter, 'J'+obj.wheelWidth].join(' ');
              break;

            case 'tyres':
              title = [obj.tyreWidth, obj.tyreHeight, 'R'+obj.diameter].join(' ');
              break;

            case 'spaces':
              title = [obj.pcdSpacesFrom, obj.pcdSpacesTo, 'ET'+obj.spacesWidth].join(' ');
              break;
          }
        }
